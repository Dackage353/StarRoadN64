...
static s32 bhv_cmd_randomize_object(void) {
    ...
    if (gCurrLevelNum != LEVEL_HMC && gCurrentObject->behavior == bhvBowserBomb)
    {
        goto end;
    }
    ...
    tinymt32_init(&randomState, Randomizer_gGameSeed + (gCurrentObject->pointerSeed/4));

    // Stuff for exclamation boxes when key objects only is enabled
    if ((Randomizer_gOptionsSettings.gameplay.s.objectRandomization == 0) && (gCurrentObject->behavior == segmented_to_virtual(bhvExclamationBox))) {
        s32 bparam = -1;
        ...
        if (0) { // if ((gCurrLevelNum == LEVEL_JRB && gCurrAreaIndex == 2) || (gCurrLevelNum == LEVEL_PSS)) {
        ...
        if (bparam != -1) {
            ...
            struct Object *star = spawn_object_abs_with_rot(gCurrentObject, 0, MODEL_STAR, bhvStar, gCurrentObject->oPosX, gCurrentObject->oPosY, gCurrentObject->oPosZ, 0, 0, 0);
...
#if 0
    // Funny moneybag
    if ((Randomizer_gOptionsSettings.gameplay.s.safeSpawns != SPAWN_SAFETY_SAFE) && (gCurrentObject->behavior == segmented_to_virtual(bhvYellowCoin))) {
        f32 rand = tinymt32_generate_float(&randomState);
        f32 chance = Randomizer_gOptionsSettings.gameplay.s.safeSpawns == SPAWN_SAFETY_HARD ? (1 / 50.f) : (1 / 200.f);
    ...
    if (((Randomizer_gOptionsSettings.gameplay.s.objectRandomization != 0) || (randType & RAND_TYPE_IMPORTANT)) && ((Randomizer_gOptionsSettings.gameplay.s.randomizeStarSpawns) || !(randType & RAND_TYPE_RANDO_STAR))) {
#if 0
        ...
        if (randType & RAND_TYPE_MAX_VARIATION) {
            ...
            Randomizer_get_safe_position(gCurrentObject, pos, 300.f, height, &randomState, (randType & RAND_TYPE_SAFE ? Randomizer_FLOOR_SAFETY_MEDIUM : Randomizer_FLOOR_SAFETY_LOW), randType);
            randomize = TRUE;
        }
        // Grounded
        else if (randType & RAND_TYPE_GROUNDED) {
            Randomizer_get_safe_position(gCurrentObject, pos, 0.f, 0.f, &randomState, Randomizer_FLOOR_SAFETY_HIGH, randType);
            randomize = TRUE;
        }
        // Min variation
            ...
            case Randomizer_SPAWN_SAFETY_SAFE:
                height = 100.f;
                break;
            case Randomizer_SPAWN_SAFETY_DEFAULT:
                height = 170.f;
                break;
            case Randomizer_SPAWN_SAFETY_HARD:
                height = 250.f;
            }
            Randomizer_get_safe_position(gCurrentObject, pos, 50.f, height, &randomState, (randType & RAND_TYPE_SAFE ? Randomizer_FLOOR_SAFETY_MEDIUM : Randomizer_FLOOR_SAFETY_LOW), randType);
            randomize = TRUE;
        }
